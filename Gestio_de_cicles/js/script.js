class Cicle {
    ...
    setNumEdicions() {
        ...
        console.log(`Nombre d'edicions: ${this.numEdicions}`);
        console.log(`Darrera data d'edició: ${this.ultimaEdicio.toLocaleString()}`);
    }
    ...
    toString() {
        let modulsInfo = this.moduls.map(modul => `  - ${modul.toString()}`).join('\n');
...
${modulsInfo}`;
    ...
    calcularHoresCicle() {
        return this.moduls.reduce((totalHores, modul) => totalHores + modul.hores, 0);
    }
}
...
function calculHores(index) {
    let horesCicle = llistatCicles[index].calcularHoresCicle();
    alert(`Les hores del cicle són: ${horesCicle}h`);
}
...
function afegirCicle() {
  ...
  let editCicleIndex = document.getElementById("editCicle").value;
  ...
  if (editCicleIndex === "-1") {
    ...
    // Editar cicle
    llistatCicles[editCicleIndex].nom = nom;
    llistatCicles[editCicleIndex].categoria = categoria;
    llistatCicles[editCicleIndex].numAlumnes = numAlumnes;
    llistatCicles[editCicleIndex].abreviatura = abreviatura;
    llistatCicles[editCicleIndex].setNumEdicions();
  }
